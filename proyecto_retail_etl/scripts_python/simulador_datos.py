...
# definición de funciones para generar cada tipo de archivo con la lógica de negocio y sucia incorporada
def generar_inventario_csv(ruta_archivo: str = RUTA_INVENTARIO_CSV) -> None:
    ...
    df_inventario = pd.DataFrame(datos) # Creamos un DataFrame a partir de la lista de diccionarios
    
    # Lógica para inyectar suciedad en el inventario: duplicados y nulos (15% de la data cada uno)
    cantidad_base = len(df_inventario)
    cantidad_15_pct = max(1, int(cantidad_base * 0.15))
    
    # 1. Inyectamos Duplicados (15% de la data)
    filas_duplicadas = df_inventario.sample(n=cantidad_15_pct, replace=True, random_state=42)
    df_inventario = pd.concat([df_inventario, filas_duplicadas], ignore_index=True)
    
    # 2. Inyectamos Nulos en stock_actual (15% de la data total actual)
    indices_nulos_stock = random.sample(range(len(df_inventario)), cantidad_15_pct)
    df_inventario.loc[indices_nulos_stock, 'stock_actual'] = np.nan
    
    # 3. Inyectamos algunos nulos extras en las categorías para mayor realismo
    indices_nulos_cat = random.sample(range(len(df_inventario)), int(cantidad_15_pct / 2))
    df_inventario.loc[indices_nulos_cat, 'categoria'] = np.nan
    
    # Desordenamos el DataFrame para que la "suciedad" esté mezclada aleatoriamente
    ...
    logging.info(f"CSV: Generado catálogo de {len(df_inventario)} productos (CON {cantidad_15_pct} DUPLICADOS Y NULOS) en '{ruta_archivo}'.")
